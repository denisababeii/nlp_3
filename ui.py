...
TEST_DATASET: List[Dict[str, Any]] = [
  ...
  {
    "text": "I read a quote by SÃ¸ren Kierkegaard and one by Hannah Arendt.",
    "persons": ["SÃ¸ren Kierkegaard", "Hannah Arendt"]
  },
  {
  ...
  {
    "text": "I saw 'Einstein CafÃ©' and thought of Albert Einstein.",
  ...
  {
    "text": "Mr. JosÃ© Mourinho Jr. arrived late, but Mourinho Street was closed anyway.",
    "persons": ["JosÃ© Mourinho Jr."]
  },
  {
  ...
  {
    "text": "Hr. Lars LÃ¸kke Rasmussen deltog i mÃ¸det i dag.",
    "persons": ["Lars LÃ¸kke Rasmussen"]
  },
  {
    "text": "Fru Helle Thorning-Schmidt besÃ¸gte campus, men skiltet sagde 'Thorning Plads'.",
    "persons": ["Helle Thorning-Schmidt"]
  },
  {
    "text": "Betzy Meyers HÃ¸j i Ballerup er spÃ¦rret pÃ¥ grund af vejarbejde.",
    "persons": []
  },
  {
    "text": "Jeg sÃ¥ Mads Mikkelsen pÃ¥ plakaten, men det var bare 'Mikkelsen & SÃ¸n'.",
    "persons": ["Mads Mikkelsen"]
  },
  {
    "text": "Ms. Pia KjÃ¦rsgaard holdt tale, og bagefter gik vi forbi KjÃ¦rsgaard AllÃ©.",
    "persons": ["Pia KjÃ¦rsgaard"]
  },
  {
    "text": "Professor Niels Bohr nÃ¦vnes ofte, men her handler det om Bohr-variablen i koden.",
    "persons": ["Niels Bohr"]
  },
  {
    "text": "Vi drak kaffe pÃ¥ 'Andersen CafÃ©'â€”ikke noget med H.C. Andersen i teksten.",
    "persons": ["H.C. Andersen"]
  },
  {
    "text": "Jeg mÃ¸dte Sofie Linde og Jakob Ellemann-Jensen ved indgangen.",
  ...
  {
    "text": "Kald lige pÃ¥ frk. Karen Blixen, hvis hun er i bygningen.",
  ...
  {
    "text": "Statsminister Mette Frederiksen og Kong Frederik mÃ¸dtes til receptionen.",
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail=(
                    ...
                    f"Checked: {url}\n"
                    "Typical fixes:\n"
                    "â€¢ Start the container / service\n"
                    "â€¢ Verify port mapping (e.g., -p 8000:8000)\n"
                    "â€¢ If running remotely, check firewall / host\n\n"
            ...
            raise HTTPException(
                ...
                detail=(
                    ...
                    f"Endpoint: {url}\n\n"
                    "Typical fixes:\n"
                    "â€¢ Increase PERSON_SERVICE_TIMEOUT_SECONDS\n"
                    "â€¢ Make the service faster (e.g., smaller model / caching)\n"
                    "â€¢ Check service logs for slow LLM calls"
                ),
            )
...
app = FastAPI(
    title="Text â†’ Persons Demo Frontend",
...
<head>
  ...
  <title>Text â†’ Persons (Demo Frontend)</title>
  <style>
    :root {{
      --dtu-red: rgb(153,0,0);
      --bg: #ffffff;
...
<body>
  <header>
    <h1>Text â†’ Persons (Demo Frontend)</h1>
    <div class="sub">
      Calls <code>{SERVICE_BASE_URL}{SERVICE_ENDPOINT}</code> â€¢ Timeout: <code>{SERVICE_TIMEOUT_SECONDS:.1f}s</code>
    </div>
  </header>
  ...
  <main>
    ...
    <div class="grid2">
      <section class="card">
        ...
        <div style="margin-top: 10px;">
          <pre id="testOutput">Press â€œRun testsâ€.</pre>
        </div>
      </section>
      ...
      <section class="card">
        ...
        <div style="margin-top: 10px;">
          <pre id="metricsOutput">Loadingâ€¦</pre>
        </div>
  ...
  <footer>
    Minimal JS + inline CSS by design (pedagogical). If you need CORS, prefer calling the service through this frontendâ€™s
...
<script>
  ...
  async function doExtract() {{
    ...
    btn.disabled = true;
    setStatus("Calling serviceâ€¦", "hint");
    try {{
      ...
      setStatus(`OK â€¢ latency ${{ms}} ms`, "ok");
    }} catch (e) {{
      renderPersons([]);
  ...
  async function runTests() {{
    ...
    btn.disabled = true;
    document.getElementById("testOutput").textContent = "Running testsâ€¦";
    try {{
